...
# 1. 載入數據
df_info = fetch_data("1251336110")   # Boar info 分頁
df_history = fetch_data("1428367761") # BOAR 歷史分頁
...
if (df_info is not None and df_history is not None) and search_id:
    ...
    if not info_match.empty:
        ...
        with f3:
            # 確保欄位名稱與您的 Excel 匹配 (Impurities, Volume)
            st.metric("⚠️ 6. Impurities (%)", str(info_data.get('Impurities', 'N/A')))
        with f4:
            st.metric("🥛 7. History Volume", str(info_data.get('Volume', 'N/A')))

        # 顯示週次趨勢 (Breed, Gen, Tag ID, W05-W01)
        st.markdown("### Weekly Usage History / Sejarah Mingguan")
        ...
        st.error(f"找不到 ID: {search_id}。請檢查 Boar info 內的 Tag ID。")
else:
    if not search_id:
        st.info("💡 系統已就緒。請輸入公豬耳號。")
